...
public:
  ...
  void resize(size_type new_size, T const& default_value);
...
private:
  ...
  void unchecked_reserve(size_type new_capacity); 
...
template<typename T>
void Vector<T>::unchecked_reserve(size_type new_capacity)
{
...
void Vector<T>::reserve(size_type new_capacity)
{
  ...
  unchecked_reserve(new_capacity);
}

template<typename It>
void destroy(It b, It e)
{
...
template <typename T>
void Vector<T>::resize(size_type new_size, T const& default_value)
{
  reserve(new_size);

  if (size() < new_size) {
    std::uninitialized_fill(end(), begin() + new_size, default_value);
  } else {
      destroy(begin() + new_size, end());
  }

  base.end = base.contents + new_size;
}
...
T& Vector<T>::emplace_back(Args&&... args)
{
  ...
  //new (base.end) T{args...};
  auto elem = new (base.end) T{std::forward<Args>(args)...};
  ++base.end;
